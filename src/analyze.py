"""
Post-training grokking analysis using saved checkpoints.

This module analyzes internal signals (representation rank, attention entropy)
across training checkpoints to understand the grokking transition.

IMPORTANT: No code executes on import. Analysis only runs when main() is called.
...
class GrokkingAnalyzer:
    """
    Analyzer for post-training grokking detection using saved checkpoints.
    """
    ...
    def __init__(self, experiment_dir: str, device: Optional[torch.device] = None):
        ...
        # Create dataloader for consistent evaluation
    ...
    def analyze_checkpoint(self, step: int) -> Dict:
        ...
        avg_entropy = np.mean(entropies)  # entropies is already a list of floats
    ...
    def run_analysis(self, t_grok: Optional[int] = None) -> Dict:
        ...
        """
        print("="*70)
        print("POST-TRAINING GROKKING ANALYSIS")
        print("="*70)
        print()
        ...
        # Step 3-4: Analyze each checkpoint
        ...
        # Step 5: Align with performance metrics
...
def main():
    """
    Main entry point for post-training analysis.

    Run this to analyze saved checkpoints and identify grokking signals.
    """
    # Configuration
    experiment_dir = 'experiments/modular_arithmetic'
    ...
    # Create analyzer
    analyzer = GrokkingAnalyzer(experiment_dir)
